...
from summertest.utils.parser import Format
...
class VariablesView(GenericViewSet):
    ...
    def update(self, request, **kwargs):
        ...
        if models.Variables.objects.exclude(id=variable_id).filter(project_id=project_id,
                                                                   key=request.data['key']).first():
            return Response(response.VARIABLES_EXISTS)

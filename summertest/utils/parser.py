...
from summertest.utils.tree import get_all_ycatid, get_tree_max_id, get_tree_ycatid_mapping
...
def format_summary_to_ding(msg_type, summary, report_name=None):
    ...
    if msg_type == 'markdown':
        ...
        # 报告地址
        report_url = f'http://127.0.0.1:8000/api/summertest/reports/{report_id}/'
...
class Yapi:
    ...
    def __init__(self,
                 yapi_base_url: str,
                 token: str,
                 summertest_project_id: int):
        self.__yapi_base_url = yapi_base_url
        self.__token = token
        self.summer_project_id = summertest_project_id
        self.api_info: list = []
        self.api_ids: list = []
        # api基础信息，不包含请求报文
    ...
    def get_api_info_list(self):
        ...
        """
        try:
            res = requests.get(self.api_list_url, params={'token': self.__token, 'page': 1, 'limit': 100000}).json()
            if res['errcode'] == 0:
                return res
        except Exception as e:
    ...
    def yapi2sumer(self, source_api_info):
        """
        yapi单个api转成summer格式
        """
    ...
    def get_parsed_apis(self, api_info):
        ...
        :return:
        """
        apis = [self.yapi2sumer(api) for api in api_info if isinstance(api, dict) is True]
        proj = models.Project.objects.get(id=self.summer_project_id)
        obj = models.Relation.objects.get(project_id=self.summer_project_id, type=1)
        eval_tree: list = eval(obj.tree)
